...
async function bootstrap() {
  ...
  try {
    ...
    console.log(
      '데이터베이스에서 이미지 마이그레이션이 필요한 문제 불러오는 중...',
    );
    ...
    const questionsToProcess = isTest
      ? targetQuestions.slice(0, 1)
      : targetQuestions;
    ...
    console.log(
      `총 ${questionsToProcess.length}개의 문제를 갱신합니다.${isTest ? ' (테스트 모드: 1개만 실행)' : ''}`,
    );
    let updatedCount = 0;
        ...
        for (const choice of q.choices) {
          if (choice.imageUrls && choice.imageUrls.length > 0) {
            ...
            for (let idx = 0; idx < choice.imageUrls.length; idx++) {
              ...
              if (!url.includes(publicDomain)) {
                console.log(
                  `[문제 ${q.id}] 보기 ${choice.number} 이미지 ${idx + 1} 업로드 중...`,
                );
    ...
    console.log(
      `   - 총 ${updatedCount}개의 문제 레코드가 업데이트되었습니다.`,
    );

    if (failedLogs.length > 0) {
      console.log('');
      console.log(
        `⚠️ ${failedLogs.length}개의 이미지 업로드 실패. 로그를 저장합니다.`,
      );
      ...
      fs.writeFileSync(
        logFile,
        JSON.stringify([...existingLogs, ...failedLogs], null, 2),
      );
      console.log(`📝 실패 로그 저장 위치: ${logFile}`);
      console.log(
        `💡 실패한 이미지들은 R2 링크를 갖지 못했기 때문에, 다음 번에 이 스크립트를 다시 실행하면 자동으로 재시도 대상(타겟)에 포함되어 전부 재시도됩니다.`,
      );
    }
  } catch (error: any) {
    console.log('');
