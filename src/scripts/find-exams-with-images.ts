...
async function bootstrap() {
    ...
    try {
        ...
        for (const q of questions) {
            ...
            if (q.question_image_urls && q.question_image_urls.some(url => !url.includes(publicDomain))) {
            ...
            // 보기에 이미지가 있고 R2 링크가 아닌 경우
                ...
                const hasExternalChoiceImage = q.choices.some(
                    c => c.imageUrls && c.imageUrls.some(url => !url.includes(publicDomain))
                );
        ...
        if (targetExamIds.size === 0) {
            console.log('🎉 재크롤링이 필요한 시험(이미지가 포함된 시험)이 없습니다!');
            return;
        }

        console.log('');
        console.log(`⚠️  총 ${targetExamIds.size}개의 시험(exam_id)에 만료된 이미지 링크가 포함되어 있습니다.`);
        console.log(`   아래의 시험 ID들을 사용하여 사이트에서 다시 '--retry' 옵션으로 크롤링해야 합니다.\n`);
        ...
        for (const examId of targetExamIds) {
            ...
            console.log(`- 시험 ID: ${examId} | 과목: ${subjectName} | 제목: ${exam?.title || '알 수 없음'} | 연도: ${exam?.year}`);
        }

        console.log('');
        console.log('💡 안내:');
        console.log('이 시험들만 실제 Tistory 웹사이트 URL을 찾아 다음과 같이 명령어를 실행하세요:');
        console.log('yarn crawl <https://allaclass.tistory.com/포스팅번호> --retry');
