...
export function parseExamType(examTypeText: string): ExamType {
  ...
  //기말시험 체크
  if (text.includes('기말')) {
    if (text.includes('2학기') || text.includes('2 학기'))
      return ExamType.SECOND_SEMESTER_FINAL;
    if (text.includes('1학기') || text.includes('1 학기'))
      return ExamType.FIRST_SEMESTER_FINAL;
    throw new Error(
      `기말시험의 학기 정보를 특정할 수 없습니다: ${examTypeText}`,
    );
  }
