...
| 번호 | 구분 | 설명 |
| --- | --- | --- |
| 21~22 | 피드백 | 피드백 제출(GitHub Issue 자동 생성), 남은 제출 횟수 조회 |
| 23~29 | 시험지 등록 | 사용자 업로드, 사전 중복 확인, 관리자 검수/수정/게시/반려 |
| 30~32 | 업데이트 알림 | 활성 공지 조회, 내역 적재(관리자), 공지 발행(관리자) |
| 33 | 마이페이지 | 최근 시험 풀이 기록 조회 (+ 기존 #11 제출 API 동작 변경) |
| 34~36 | 북마크 | 목록 조회, 등록, 해제 |
| 37 | 문항 | 문항 단건 공개 조회 (암기모드 공유 진입점) |

**공통 규칙**
- 응답 포맷은 기존과 동일하게 `{ success: boolean, data: ... }` 기본, 필요 시 최상위에 부가 필드(`remainingCount` 등) 추가.
...
| **Method** | POST |
| **URL** | /api/feedbacks |
| **설명** | 피드백/문항 이상 제보를 접수합니다. 접수 즉시 전용 GitHub 레포에 Issue를 자동 생성하고 Discord 웹훅으로 알림을 보냅니다. GitHub/Discord 연동이 실패해도 접수(DB 저장) 자체는 성공 처리됩니다. **로그인 필수, 일일 제출 횟수 제한**(스팸 방지) |
...
| remainingCount | 오늘 남은 제출 가능 횟수 | number | - | N |
...
| 401 | 인증 실패 (로그인 필요) |
| 403 | 일일 제출 횟수 초과 (일일 15회) |
| 404 | questionId에 해당하는 문항을 찾을 수 없음 |

---

## 22. Feedback - 남은 제출 가능 횟수 조회
...
| --- | --- |
| **Method** | GET |
| **URL** | /api/feedbacks/remaining-count |
| **설명** | 오늘 남은 피드백 제출 가능 횟수를 반환합니다. (tutor의 remaining-count와 동일 패턴) |
| **인증** | JWT Bearer Token 필수 |
...
| remainingCount | 오늘 남은 제출 가능 횟수 | number |
| totalLimit | 일일 총 제한 횟수 | number |
